...
# 根据速度计算广义角度，单位为弧度
def calc_theta(motion_states,degree=True):
    assert isinstance(motion_states,MotionState)
    vx = motion_states.vx
    vy = motion_states.vy
    change_coefficient = 1
    if not degree:
        change_coefficient = pi/180
    if vx == 0:
        return pi/2*change_coefficient if vy > 0 else 3*pi/2*change_coefficient
    if vx>0:
        return atan(vy/vx)*change_coefficient if vy>0\
            else (2*pi-abs(atan(vy/vx)))*change_coefficient
    else:
        return (pi-abs(atan(vy/vx)))*change_coefficient if vy>0\
            else (pi+atan(vy/vx))*change_coefficient
        ...
        for key, value in track_dict.items():
            ...
            if value.time_stamp_ms_first <= timestamp <= value.time_stamp_ms_last:
                ...
                if key not in patches_dict:
                    ...
                    axes.add_patch(rect)
                    text_dict[key] = axes.text(ms.x, ms.y + 2, f'{str(key)}', horizontalalignment='center', zorder=30)
                else:
                    width = value.width
                    length = value.length
